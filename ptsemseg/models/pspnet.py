...
class pspnet(nn.Module):
  ...
  def __init__(
    ...
    self.convbnrelu1_1 = conv2DBatchNormRelu(
      in_channels=1, k_size=3, n_filters=8, padding=1, stride=2, bias=False
    )
    self.convbnrelu1_2 = conv2DBatchNormRelu(
      in_channels=8, k_size=3, n_filters=8, padding=1, stride=1, bias=False
    )
    self.convbnrelu1_3 = conv2DBatchNormRelu(
      in_channels=8, k_size=3, n_filters=16, padding=1, stride=1, bias=False
    )

    # Vanilla Residual Blocks
    self.res_block2 = residualBlockPSP(self.block_config[0], 16, 8, 32, 1, 1)
    self.res_block3 = residualBlockPSP(self.block_config[1], 32, 16, 64, 2, 1)

    # Dilated Residual Blocks
    self.res_block4 = residualBlockPSP(self.block_config[2], 64, 32, 128, 1, 2)
    self.res_block5 = residualBlockPSP(self.block_config[3], 128, 64, 256, 1, 4)

    # Pyramid Pooling Module
    self.pyramid_pooling = pyramidPooling(256, [6, 3, 2, 1])

    # Final conv layers
    self.cbr_final = conv2DBatchNormRelu(512, 64, 3, 1, 1, False)
    ...
    #   in_channels=128, k_size=3, n_filters=32, padding=1, stride=1, bias=False
    # )
    # self.aux_cls = nn.Conv2d(32, self.n_classes, 1, 1, 0)
  ...
  def tile_predict(self, imgs, include_flip_mode=True):
    ...
    score = (pred / count[None, None, ...]).astype(np.float8)
