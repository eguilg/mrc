...
def gen_ans(pos1, pos2, raw_data):
	pred_list = raw_data['article_tokens'][pos1:pos2 + 1]
	...
	if len(pred_text) > 0 and re.match('[0-9]+', pred_text[-1]):
		if pos2 + 1 < len(raw_data['article_tokens']) and raw_data['article_tokens'][pos2 + 1] in unit_set:
			pos2 += 1
			pred_text += raw_data['article_tokens'][pos2]
